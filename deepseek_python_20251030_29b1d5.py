...
class RegattaExtractor:
    def __init__(self, root):
        self.root = root
        self.root.title("Regatta Daten Extraktor - PDF Import (Verbessert)")
        self.root.geometry("1000x800")
        
        # Datenbank-Pfad
        self.db_dir = r"C:\Ablage_C\python_work\Git_Test630Nov" 
    ...
    def create_widgets(self):
        ...
        ttk.Button(main_frame, text="Debug Extraktion", 
                  command=self.debug_extraction_process).grid(row=3, column=1, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Als Text exportieren", 
                  command=self.export_data_txt).grid(row=3, column=2, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Als CSV exportieren", 
                  command=self.export_data_csv).grid(row=3, column=3, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Datenbank löschen", 
                  command=self.delete_database).grid(row=4, column=0, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Alle Zeilen anzeigen", 
                  command=self.show_all_data).grid(row=4, column=1, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Statistik anzeigen", 
                  command=self.show_statistics).grid(row=4, column=2, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Duplikate analysieren", 
                  command=self.analyze_duplicates).grid(row=4, column=3, pady=5, padx=2)
        
        ttk.Button(main_frame, text="Datenbank reparieren", 
                  command=self.repair_database).grid(row=5, column=0, pady=5, padx=2)
        
        # Progress Bar
        self.progress = ttk.Progressbar(main_frame, mode='determinate')
        self.progress.grid(row=6, column=0, columnspan=4, pady=5, sticky=(tk.W, tk.E))
        ...
        text_frame.grid(row=8, column=0, columnspan=4, pady=10, sticky=(tk.W, tk.E, tk.N, tk.S))
    ...
    def extract_text_from_pdf(self, pdf_path):
        """Extrahiert Text mit pdfplumber und versucht Tabellen zu erkennen"""
        try:
            all_text_lines = []
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages):
                    ...
                    # VERBESSERTE TEXTTEXTRAKTION
                    # 1. Normale Textextraktion
                    ...
                    # 2. Tabellenextraktion versuchen
                    try:
                        tables = page.extract_tables()
                        for table in tables:
                            for row in table:
                                if row and any(cell is not None for cell in row):  # Nicht-leere Zeilen
                                    # Kombiniere Zellen zu einer lesbaren Zeile
                                    table_line = " | ".join([str(cell).strip() for cell in row if cell])
                                    if table_line and table_line.strip():
                                        all_text_lines.append("TABLE: " + table_line)
                    except Exception as table_error:
                        print(f"Tabellenextraktion fehlgeschlagen auf Seite {page_num + 1}: {table_error}")
                    
                    # 3. Alternative Extraktion mit extract_text_layout
                    try:
                        layout_text = page.extract_text_layout()
                        if layout_text:
                            layout_lines = layout_text.split('\n')
                            for line in layout_lines:
                                if line.strip() and line not in all_text_lines:
                                    all_text_lines.append("LAYOUT: " + line)
                    except Exception as layout_error:
                        print(f"Layout-Extraktion fehlgeschlagen: {layout_error}")
                    
                    # Seitenfuß hinzufügen
    ...
    def looks_like_table_data(self, line):
        """Prüft, ob es sich um Tabellendaten handelt"""
        table_patterns = [
            r'^\d+\s+\d+\s+',  # Beginnt mit Zahlen (Rang + Startnummer)
            r'^\|\s*\d+\s*\|',  # Tabellenformat mit Pipe-Zeichen
            r'\s+\d+:\d+\.\d+\s*[\+\-]?',  # Enthält Zeitformat
            r'^Rang\s+StNr\s+Boot',  # Tabellenheader
            r'^\d+\s+\d+\s+[A-Za-z]',  # Rang + StNr + Text
            r'TABLE:',  # Von Tabellenextraktion markiert
            r'LAYOUT:'  # Von Layout-Extraktion markiert
        ]
    ...
    def clean_extracted_text(self, lines):
        """Bereinigt den extrahierten Text mit weniger aggressiver Duplikaterkennung"""
        cleaned_lines = []
        seen_content = set()
        ...
        for line in lines:
            ...
            # Immer Seitenmarkierungen einfügen
            if self.is_page_marker(original_line):
                cleaned_lines.append(original_line)
                continue
                
            # Weniger aggressive Duplikaterkennung für Tabellendaten
            if self.looks_like_table_data(original_line):
                # Für Tabellendaten: Erlaube mehr Duplikate
                if normalized_key not in seen_content or len(cleaned_lines) < 10:  # Erste Zeilen immer behalten
                    cleaned_lines.append(original_line)
                    seen_content.add(normalized_key)
            else:
                # Normale Duplikatprüfung
    ...
    def debug_extraction_process(self):
        """Debug-Funktion zum Überprüfen des Extraktionsprozesses"""
        if not self.pdf_file_path:
            messagebox.showwarning("Warnung", "Bitte wählen Sie zuerst eine PDF-Datei aus!")
            return
        
        try:
            self.status_label.config(text="Starte Debug-Extraktion...")
            ...
            self.text_area.insert(tk.END, "=== DEBUG EXTRAKTION ===\n\n")
            
            self.text_area.insert(tk.END, f"Original: {len(pdf_lines)} Zeilen\n")
            self.text_area.insert(tk.END, f"Bereinigt: {len(cleaned_lines)} Zeilen\n")
            self.text_area.insert(tk.END, f"Entfernt: {len(pdf_lines) - len(cleaned_lines)} Zeilen\n\n")
            
            # Zeige die ersten Unterschiede
            self.text_area.insert(tk.END, "ERSTE 30 ORIGINALZEILEN:\n")
            self.text_area.insert(tk.END, "=" * 80 + "\n")
            for i, line in enumerate(pdf_lines[:30]):
                marker = "✓" if line in cleaned_lines else "✗"
                self.text_area.insert(tk.END, f"{i+1:3d} {marker}: {line}\n")
            
            self.text_area.insert(tk.END, "\nERSTE 30 BEREINIGTE ZEILEN:\n")
            self.text_area.insert(tk.END, "=" * 80 + "\n")
            for i, line in enumerate(cleaned_lines[:30]):
                self.text_area.insert(tk.END, f"{i+1:3d}: {line}\n")
            
            # Zeige Tabellendaten
            table_lines = [line for line in cleaned_lines if self.looks_like_table_data(line)]
            self.text_area.insert(tk.END, f"\nGEFUNDENE TABELLENDATEN: {len(table_lines)} Zeilen\n")
            self.text_area.insert(tk.END, "=" * 80 + "\n")
            for i, line in enumerate(table_lines[:20]):
                self.text_area.insert(tk.END, f"{i+1:3d}: {line}\n")
            
            self.status_label.config(text=f"Debug abgeschlossen - {len(table_lines)} Tabellenzeilen")
            
        except Exception as e:
            messagebox.showerror("Fehler", f"Fehler bei Debug-Extraktion: {str(e)}")
    ...
    def import_from_pdf(self):
        ...
        try:
            ...
            for line in cleaned_lines:
                ...
                if self.looks_like_table_data(line):
                    table_data_count += 1
            ...
            stats_text += f"- {table_data_count} Tabellendatenzeilen\n"
            stats_text += f"- {len(cleaned_lines) - page_markers - table_data_count} andere Zeilen\n"
        ...
        try:
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    elif self.looks_like_table_data(content):
                        line_type = "TABLE"
    ...
    def show_statistics(self):
        ...
        try:
            ...
            # Zähle Tabellendaten
            cursor.execute("SELECT COUNT(*) FROM regatta_daten WHERE Regatta = ? AND ErgZei LIKE 'TABLE:%'", (self.regatta_name,))
            table_data = cursor.fetchone()[0]
            ...
            self.text_area.insert(tk.END, f"Tabellendaten: {table_data}\n")
